...
-- Eccezione beta nominativa: consente all'account proprietario di aprire la
-- sessione senza prova GPS/QR. Restano obbligatori autenticazione e ruolo
-- legittimo nell'evento; non vengono creati check-in ne assegnate ricompense.
...
declare
  ...
  location_optional boolean := lower(coalesce(auth.jwt() ->> 'email', '')) = 'aletarqui@libero.it';
begin
  ...
  if target_event.creator_id = actor_id then
    ...
    if not found
      ...
      or (participant.checked_in_at is null and not location_optional)
    then
