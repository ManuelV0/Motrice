...
test('virtual wallet starts with two trial events and no real balance', () => {
...
test('virtual top-up is fixed and idempotent', () => {
  window.localStorage.clear();
  piggybank.claimVirtualCredit({ requestId: 'request-12345678' });
  const retry = piggybank.claimVirtualCredit({ requestId: 'request-12345678' });
  assert.equal(retry.available_cents, 1000);
  assert.equal(retry.history.filter((entry) => entry.entry_type === 'virtual_credit_added').length, 1);
});

test('event holds consume trials first, then lock and restore virtual credit', () => {
  window.localStorage.clear();
  piggybank.claimVirtualCredit({ requestId: 'request-balance-1' });
...
test('cancel restores a trial while a no-show consumes the held value', () => {
  ...
  piggybank.claimVirtualCredit({ requestId: 'request-no-show-1' });
  ...
  assert.equal(wallet.available_cents, 0);
  assert.equal(wallet.locked_cents, 0);
  assert.equal(wallet.total_cents, 0);
});
